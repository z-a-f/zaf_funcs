...
class Reuters(_BaseDataset):
  """Reuters newswire documents classification dataset.
  ...
  def __init__(self, path=None, num_words=10000, **kwargs):
    if path is None:
      path = os.path.join(os.getcwd(), "data/Reuters")
    super().__init__(path)
  ...
  def load(self, num_words, **kwargs):
    ...
    """
    self._loader(tf_ds.reuters, num_words, **kwargs)
